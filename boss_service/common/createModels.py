...
class Models(object):
    ...
    def datafl(self):
        ...
        self.intList = ",".join(self.intList)
        return all_fields, init_fields, db_column, fileds, listfields
        ...
        if tablename != []:
            for table in tablename:
                if table in table_list:
                    ...
                    try:
                        all_fields, init_fields, db_column, fileds, listfields = self.datafl()
                        tablename = table
                        modelName = self.getName(tablename)
                        returnMsg = "'{} : %s' % self.{}".format(self.col_name_list[0], self.col_name_list[0])
                        tableChangeDic = "{}ChangeDic".format(modelName)
                        models = models.format(tableChangeDic=tableChangeDic,model_Name=modelName, table_name=tablename, all_fields=db_column,
                                               fileds=fileds,
                                               initfields=init_fields, returnMsg=returnMsg, listfields=listfields,
                                               changeFields=all_fields, intLists=self.intList).replace("[","{").replace("]","}").replace("\a","[").replace("\b","]")
            ...
            for table in table_list:
                ...
                try:
                    ...
                    models = models.format(tableChangeDic=tableChangeDic,model_Name=modelName, table_name=tablename, all_fields=db_column,
                                           fileds=fileds,
                                           initfields=init_fields, returnMsg=returnMsg, listfields=listfields,
                                           changeFields=all_fields, intLists=self.intList).replace("[", "{").replace("]", "}")
                    self.intList = []
                    if self.creatModel(table, models):
                        print ("success", table)
                    else:
